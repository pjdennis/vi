...
dontcr:
	...
	while (outline < destline) {
		...
		if (j > costDP && parm_down_cursor) {
			/* Win big on Tek 4025 */
			tputs(tparm(parm_down_cursor, j, 0), j, plodput);
			outline += j;
		}
		else {
			outline++;
	...
	while (outcol > destcol) {
		...
		if (j > costLP && parm_left_cursor) {
			tputs(tparm(parm_left_cursor, j, 0), j, plodput);
			outcol -= j;
		}
		else {
			outcol--;
	...
	while (outline > destline) {
		...
		if (parm_up_cursor && j > 1) {
			/* Win big on Tek 4025 */
			tputs(tparm(parm_up_cursor, j, 0), j, plodput);
			outline -= j;
		}
		else {
			outline--;
	...
	while (outcol < destcol) {
		...
		if (j > costRP && parm_right_cursor) {
			 ...
			 */
			tputs(tparm(parm_right_cursor, j, 0), j, plodput);
			outcol += j;
		}
		else {
			/*
