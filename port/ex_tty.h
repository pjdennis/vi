 ...
 *
 * PORT_NOTE: Terminal capabilities are now provided by vi_term.h (ANSI escape sequences).
 */
...
/*
 * TTY mode handling.
 * ORIG_BEGIN: USG vs BSD tty structures
 * Original used struct termio for USG, struct sgttyb for BSD.
 * We use struct termios (via vi_compat.h) for all.
 * ORIG_END
 */
var	struct	termios tty;	/* Use this one structure to change modes */
typedef	struct termios ttymode;	/* Mode to contain tty flags */

/* ORIG_REMOVED: BSD TIOCSETC/TIOCLGET sections */
...
/* ORIG_BEGIN: VMUNIX conditional for macro limits */
/* ORIG_END */
