...
/*
 * ORIG: #include "varargs.h"
 * Modern replacement: stdarg.h
 */
#include <stdarg.h>

/*
 * This version of printf is compatible with the Version 7 C
 * printf. It prints through putchar. PORT_NOTE: (vi's own putchar, not libc's).
 *
 * ORIG: The function was named "printf" which collides with libc.
 * We keep the name since vi #undefs libc's printf and uses its own putchar.
 */
...
char *_p_dconv();
/* ORIG: static forward declarations in K&R style */
void _p_emit(char *s, char *send);

/*
 * ORIG_BEGIN: K&R varargs printf
 * printf(va_alist) va_dcl { va_list ap; ... va_start(ap); fmt = va_arg(ap, char *); ... }
 * ORIG_END
 */
/* PORT_NOTE: vi's printf. Since we undef libc's printf in ex.h and don't
 * link against stdio printf, we can keep the name. */
void
printf(char *fmt, ...)
{
	va_list ap;
