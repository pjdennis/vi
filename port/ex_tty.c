...
gettmode()
{
	...
	GT = 1;
	/* ORIG_BEGIN: USG path only (tcgetattr replaces ioctl TCGETA) */
	...
	char *type;
{
	...
	/*
	 * ORIG_REMOVED: #ifdef SIGTSTP block
	 * The original code mapped the user's suspend character (olttyc.t_suspc)
	 * to ^Z using the BSD TIOCGETD ioctl and ltchars structure.
	 * These are BSD-only features not available with termios on modern Linux.
	 * The default VSUSP (^Z) handling is done by the terminal driver.
	 */
